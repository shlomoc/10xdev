---
title: AI Development Resources — Tools, Articles & Community
description: Explore curated AI development resources including tools, MCP references, articles, learning materials, communities, and media for AI-powered software development.
---

# AI Resources

A curated collection of tools, learning materials, articles, and community resources for AI-powered development.

## Explore by Category

### [Tools & MCP](./tools/)
Directories, search tools, model deployment platforms, productivity apps, and Model Context Protocol resources.

### [Learning & Articles](./learning/)
Educational references, roadmaps, 10x development articles, and longer-form writing on AI engineering.

### [Media & Community](./community/)
YouTube channels and videos, communities, newsletters, and blogs worth following.

## Featured Resources

* [AI Tools Directory](https://tools.forwardfuture.ai/) - Forward Future's AI tools directory
* [MCP](https://modelcontextprotocol.io/introduction) - Official introduction to the Model Context Protocol
* [The Modern Software Dev](https://themodernsoftware.dev) - CS146 The Modern Software Developer
* [AI Engineer Roadmap](https://roadmap.sh/ai-engineer/) - Roadmap.sh guide for AI engineers
* [The Batch](https://www.deeplearning.ai/the-batch/) - Weekly AI news and insights
* [Simon Willison's Weblog](https://simonwillison.net/) - Blog covering applied AI, tools, and software
